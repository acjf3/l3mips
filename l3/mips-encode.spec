...
string op1i (s::string, n::bits(N)) = PadRight (#" ", 8, s) : "0x" : [n]
string op1r (s::string, n::reg) = PadRight (#" ", 8, s) : r(n)
...
string instructionToString (i::instruction) =
   match i
   {
     ...
     case Shift (SRAV (rs, rt, rd))      => op3r ("slav",rd,rt,rs)
     ...
     case MultDiv (MFLO (rs))            => op1r ("mfhi",rs)
...
word Encode (i::instruction) =
   match i
   {
     ...
     case MultDiv (MTHI (rd))            => form1 (0, 0, rd, 0, '010001')
     case MultDiv (MFLO (rs))            => form1 (rs, 0, 0, 0, '010010')
