(* mips - generated by L<3> - Tue Aug 19 21:06:23 2014 *)
...
val PIC_base_address = ref (Map.mkMap(SOME 2,BitsN.B(0x0,37)))
  : (BitsN.nbit Map.map) ref
...
val PIC_external_intrs = ref (Map.mkMap(SOME 2,BitsN.B(0x0,64)))
  : (BitsN.nbit Map.map) ref

val PIC_ip_bits = ref (Map.mkMap(SOME 2,BitsN.B(0x0,128)))
  : (BitsN.nbit Map.map) ref
...
val c_BranchDelay = ref (Map.mkMap(SOME 2,NONE))
  : (BitsN.nbit option Map.map) ref

val c_BranchTo = ref (Map.mkMap(SOME 2,NONE))
  : (BitsN.nbit option Map.map) ref
      ...
      {BadVAddr = BitsN.B(0x0,64),
       ...
       Status =
         {BEV = false, CU0 = false, ERL = false, EXL = false, FR = false,
          IE = false, IM = BitsN.B(0x0,8), KSU = BitsN.B(0x0,2),
...
val c_LLbit = ref (Map.mkMap(SOME 2,NONE)): (bool option Map.map) ref

val c_PC = ref (Map.mkMap(SOME 2,BitsN.B(0x0,64)))
  : (BitsN.nbit Map.map) ref
...
val c_exceptionSignalled = ref (Map.mkMap(SOME 2,false))
  : (bool Map.map) ref

val c_gpr = ref (Map.mkMap(SOME 2,Map.mkMap(SOME 32,BitsN.B(0x0,64))))
  : (BitsN.nbit Map.map Map.map) ref

val c_hi = ref (Map.mkMap(SOME 2,NONE)): (BitsN.nbit option Map.map) ref

val c_lo = ref (Map.mkMap(SOME 2,NONE)): (BitsN.nbit option Map.map) ref

val log = ref ([]): (event list) ref

val procID = ref (BitsN.B(0x0,1)): BitsN.nbit ref
    ...
    if not(((#RI)((#control) (!JTAG_UART))) = readIntr)
      then ( let
           ...
           ; let
               val w =
                 Map.lookup
                   ((!PIC_external_intrs),BitsN.toNat(BitsN.B(0x0,1)))
             in
               PIC_external_intrs :=
               (Map.update
                  ((!PIC_external_intrs),BitsN.toNat(BitsN.B(0x0,1)),
...
fun write'GPR (value,n) =
  if not(n = (BitsN.B(0x0,5)))
    then ( write'gpr(value,n); mark(w_gpr(n,value)) )
  else ();
...
fun write'HI value = ( write'hi(Option.SOME value); mark(w_hi value) );
...
fun write'LO value = ( write'lo(Option.SOME value); mark(w_lo value) );
...
fun CPR (n,(reg,sel)) =
  case (n,(reg,sel)) of
   ...
   | (0,(BitsN.B(0xF,5),BitsN.B(0x1,3))) =>
        BitsN.zeroExtend 64
          (BitsN.@@
             (BitsN.B(0x1,16),BitsN.fromNat(BitsN.toNat (!procID),16)))
    ...
    in
      ( if a = ((#base_address) (!JTAG_UART))
          then ( ret :=
              ...
              (a,
               Map.lookup((!PIC_base_address),BitsN.toNat(BitsN.B(0x0,1))))) andalso
           (BitsN.<
              (a,
               BitsN.+
                 (Map.lookup
                    ((!PIC_base_address),BitsN.toNat(BitsN.B(0x0,1))),
                  BitsN.B(0x430,37))))
          then ret := (PIC_load(BitsN.B(0x0,1),a))
        else if (BitsN.>=
              (a,
               Map.lookup((!PIC_base_address),BitsN.toNat(BitsN.B(0x1,1))))) andalso
           (BitsN.<
              (a,
               BitsN.+
                 (Map.lookup
                    ((!PIC_base_address),BitsN.toNat(BitsN.B(0x1,1))),
                  BitsN.B(0x430,37))))
          then ret := (PIC_load(BitsN.B(0x1,1),a))
  ...
  in
    ...
    ; if a = ((#base_address) (!JTAG_UART))
        then JTAG_UART_store(mask,MemElem)
            ...
            (a,Map.lookup((!PIC_base_address),BitsN.toNat(BitsN.B(0x0,1))))) andalso
         (BitsN.<
            (a,
             BitsN.+
               (Map.lookup
                  ((!PIC_base_address),BitsN.toNat(BitsN.B(0x0,1))),
                BitsN.B(0x430,37))))
        then PIC_store(BitsN.B(0x0,1),(a,(mask,MemElem)))
      else if (BitsN.>=
            (a,Map.lookup((!PIC_base_address),BitsN.toNat(BitsN.B(0x1,1))))) andalso
         (BitsN.<
            (a,
             BitsN.+
               (Map.lookup
                  ((!PIC_base_address),BitsN.toNat(BitsN.B(0x1,1))),
                BitsN.B(0x430,37))))
        then PIC_store(BitsN.B(0x1,1),(a,(mask,MemElem)))
      else ( L3.for
               (0,1,
                fn core =>
                  if ((not(core = (BitsN.toNat (!procID)))) andalso
                      ((Map.lookup
                          ((!c_LLbit),BitsN.toNat(BitsN.fromNat(core,1)))) =
                       (Option.SOME true))) andalso
                     ((BitsN.bits
                         ((#LLAddr)
                            (Map.lookup
                               ((!c_CP0),
                                BitsN.toNat(BitsN.fromNat(core,1)))),39,3)) =
                      (BitsN.bits(pAddr,39,3)))
                    then let
                           val x = BitsN.fromNat(core,1)
                         in
                           c_LLbit :=
                           (Map.update
                              ((!c_LLbit),BitsN.toNat x,Option.SOME false))
                         end
                  else ())
           ; MEM :=
             (Map.update
                ((!MEM),BitsN.toNat a,
                 BitsN.||
                   (BitsN.&&
                      (Map.lookup((!MEM),BitsN.toNat a),BitsN.~ mask),
                    BitsN.&&(MemElem,mask))))
           )
    )
  end;
...
fun initMips (pc,uart) =
  ...
  ; let
      ...
      val x0 = (#Config1) x
    in
      write'CP0(CP0_Config1_rupd(x,ConfigRegister1_C2_rupd(x0,false)))
    end
  ; let
      val x = CP0 ()
      val x0 = (#Config1) x
  ...
  ; if (!procID) = (BitsN.B(0x0,1))
      then MEM := (Map.mkMap(SOME 137438953472,BitsN.B(0x0,64)))
    else ()
  ; L3.for
      (0,31,
       fn i =>
  ...
  ; if (!procID) = (BitsN.B(0x0,1))
      then PIC_initialise 2139111424
    else PIC_initialise 2139127808
  );
