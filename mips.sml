(* mips - generated by L<3> - Thu Jul 24 09:22:57 2014 *)
...
fun LookupTLB (r,vpn2) =
  let
    val index = BitsN.-(BitsN.bits(vpn2,6,0),BitsN.fromNat(TLBEntries,7))
    val e = Map.lookup((!TLB_direct),BitsN.toNat index)
  ...
  in
    let
      val found = ref (if (((BitsN.&&((#VPN2) e,nmask)) =
           ...
           then [(BitsN.+
                    (BitsN.fromNat(TLBEntries,8),
                     BitsN.fromNat(BitsN.toNat index,8)),e)]
         else [])
    in
      ( L3.for
          (0,Nat.-(TLBEntries,1),
...
fun dfn'TLBR () =
  if (not((#CU0)((#Status) (!CP0)))) andalso (not(KernelMode ()))
    then SignalException CpU
       ...
       in
         if BitsN.>=(i,BitsN.fromNat(TLBEntries,8))
...
fun dfn'TLBWI () =
  if (not((#CU0)((#Status) (!CP0)))) andalso (not(KernelMode ()))
    ...
    then let
           val j =
             BitsN.-
               (BitsN.bits((#VPN2)((#EntryHi) (!CP0)),6,0),
                BitsN.fromNat(TLBEntries,7))
         in
           TLB_direct :=
           (Map.update
