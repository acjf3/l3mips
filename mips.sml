(* mips - generated by L<3> - Mon Jul 21 15:01:19 2014 *)
...
fun LookupTLB (r,vpn2) =
  let
    val index = BitsN.-(BitsN.bits(vpn2,6,0),BitsN.B(0x10,7))
  ...
  in
    let
      val found = ref (if (((BitsN.&&((#VPN2) e,nmask)) =
           ...
           then [(BitsN.+
                    (BitsN.B(0x10,8),BitsN.fromNat(BitsN.toNat index,8)),e)]
         else [])
    in
      ( L3.for
          (0,Nat.-(TLBEntries,1),
...
fun dfn'TLBWI () =
  if (not((#CU0)((#Status) (!CP0)))) andalso (not(KernelMode ()))
    ...
    then let
           val j =
             BitsN.-
               (BitsN.bits((#VPN2)((#EntryHi) (!CP0)),6,0),BitsN.B(0x10,7))
         in
           TLB_direct :=
           (Map.update
