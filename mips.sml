(* mips - generated by L<3> - Sat Apr 26 11:47:34 2014 *)
...
fun SignalException ExceptionType =
  ( if (#IE)((#Status) (!CP0))
      then ( if not((#EXL)((#Status) (!CP0)))
               then if Option.isSome (!BranchDelay)
                      then ( CP0 := (CP0_EPC_rupd((!CP0),(!PC)))
                           ; let
                               val x0 = (#Cause) (!CP0)
                             in
                               CP0 :=
             ...
             in
               ...
               ; let
                 ...
                 in
                   CP0 :=
                   (CP0_Status_rupd
                      ((!CP0),StatusRegister_EXL_rupd(x0,true)))
                 end
               ; let
                   val vectorBase =
                     if (#BEV)((#Status) (!CP0))
    ...
    else ( BranchTo := NONE; PC := (BitsN.+((!PC),BitsN.B(0x4,64))) )
...
fun LoadMemory (CCA,(AccessLength,(pAddr,(vAddr,IorD)))) =
  ...
  in
    let
      val d = ref (Map.lookup((!MEM),BitsN.toNat a))
    in
      let
        val b = BitsN.toNat(BitsN.bits(pAddr,2,0))
      in
        ( if ((a = ((#base_address) (!JTAG_UART))) andalso (Nat.<(b,4))) andalso
             (Nat.<=(4,Nat.+(b,BitsN.toNat AccessLength)))
            then ( case (#read_fifo) (!JTAG_UART) of
                      [] =>
                         ( let
                         ...
                         ; let
                           ...
                           in
                             ...
                             (JTAG_UART_data_rupd
                                ((!JTAG_UART),
                                 JTAG_UART_data_RVALID_rupd(x0,false)))
                           end
                         )
                    | h :: t =>
                       ( let
                         ...
                         in
                           ...
                           (JTAG_UART_data_rupd
                              ((!JTAG_UART),
                               JTAG_UART_data_RW_DATA_rupd(x0,h)))
                         end
                       ; let
                           val x0 = (#data) (!JTAG_UART)
                         in
                       ...
                       ; let
                         ...
                         in
                           ...
                           (JTAG_UART_data_rupd
                              ((!JTAG_UART),
                               JTAG_UART_data_RVALID_rupd(x0,true)))
                         end
                       ; JTAG_UART :=
                         (JTAG_UART_read_fifo_rupd((!JTAG_UART),t))
                       )
                 ; let
...
fun StoreMemory (CCA,(AccessLength,(MemElem,(pAddr,(vAddr,IorD))))) =
  ...
  in
    ...
    ; if a = ((#base_address) (!JTAG_UART))
        then ( if not((BitsN.bits(mask,39,32)) = (BitsN.B(0x0,8)))
                 then ( let
                          val x0 = (#data) (!JTAG_UART)
                        in
                          JTAG_UART :=
             ...
             ; if BitsN.bit(mask,0)
                 then let
                      ...
                      in
                        ...
                        (JTAG_UART_control_rupd
                           ((!JTAG_UART),
                            JTAG_UART_control_RE_rupd
                              (x0,BitsN.bit(MemElem,0))))
                      end
               else ()
             ; if BitsN.bit(mask,1)
                 then let
                      ...
                      in
                        ...
                        (JTAG_UART_control_rupd
                           ((!JTAG_UART),
                            JTAG_UART_control_WE_rupd
                              (x0,BitsN.bit(MemElem,1))))
                      end
               else ()
             ; if (BitsN.bit(mask,10)) andalso (BitsN.bit(MemElem,10))
                 then let
                        val x0 = (#control) (!JTAG_UART)
                      in
                        JTAG_UART :=
                        (JTAG_UART_control_rupd
                           ((!JTAG_UART),
                            JTAG_UART_control_AC_rupd(x0,false)))
                      end
               else ()
             ; let
                 val x0 = (#control) (!JTAG_UART)
