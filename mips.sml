(* mips - generated by L<3> - Wed Aug 13 15:26:28 2014 *)
...
type PIC =
  { base_address: BitsN.nbit, config_regs: PIC_Config_Reg Map.map,
    external_intrs: BitsN.nbit, ip_bits: BitsN.nbit,
    mips_ip_bits: BitsN.nbit }
...
fun PIC_base_address_rupd ({base_address, config_regs, external_intrs,
   ip_bits, mips_ip_bits}: PIC, x') =
  {base_address = x', config_regs = config_regs,
   external_intrs = external_intrs, ip_bits = ip_bits,
   mips_ip_bits = mips_ip_bits}: PIC

fun PIC_config_regs_rupd ({base_address, config_regs, external_intrs,
   ip_bits, mips_ip_bits}: PIC, x') =
  {base_address = base_address, config_regs = x',
   external_intrs = external_intrs, ip_bits = ip_bits,
   mips_ip_bits = mips_ip_bits}: PIC

fun PIC_external_intrs_rupd ({base_address, config_regs, external_intrs,
   ip_bits, mips_ip_bits}: PIC, x') =
  {base_address = base_address, config_regs = config_regs,
   external_intrs = x', ip_bits = ip_bits, mips_ip_bits = mips_ip_bits}
  : PIC

fun PIC_ip_bits_rupd ({base_address, config_regs, external_intrs, ip_bits,
   mips_ip_bits}: PIC, x') =
  {base_address = base_address, config_regs = config_regs,
   external_intrs = external_intrs, ip_bits = x',
   mips_ip_bits = mips_ip_bits}: PIC

fun PIC_mips_ip_bits_rupd ({base_address, config_regs, external_intrs,
   ip_bits, mips_ip_bits}: PIC, x') =
  {base_address = base_address, config_regs = config_regs,
   external_intrs = external_intrs, ip_bits = ip_bits, mips_ip_bits = x'}
  : PIC
...
val PIC = ref
  ({base_address = BitsN.B(0x0,37),
    ...
    external_intrs = BitsN.B(0x0,64), ip_bits = BitsN.B(0x0,128),
    mips_ip_bits = BitsN.B(0x0,6)}): PIC ref
...
fun rec'ConfigRegister1 x =
  {C2 = BitsN.bit(x,6), CA = BitsN.bit(x,2), DA = BitsN.bits(x,9,7),
   ...
   MMUSize = BitsN.bits(x,30,25), PC = BitsN.bit(x,4), WR = BitsN.bit(x,3)};
...
fun PIC_update () =
  let
    val ext = BitsN.zeroExtend 128 ((#external_intrs) (!PIC))
  in
    let
      val ip = ref (BitsN.B(0x0,8))
    in
      ( ip := (BitsN.B(0x0,8))
      ; if (not(((#ip_bits) (!PIC)) = (BitsN.B(0x0,128)))) orelse
           (not(ext = (BitsN.B(0x0,128))))
          then L3.for
                 (0,127,
                  fn i =>
                    if (BitsN.bit((#ip_bits) (!PIC),i)) orelse
                       (BitsN.bit(ext,i))
                      then let
                             val reg =
                               Map.lookup
      ...
      ; PIC := (PIC_mips_ip_bits_rupd((!PIC),BitsN.bits((!ip),5,0)))
      )
    end
  end;
...
fun PIC_initialise pic =
  ( PIC :=
    (PIC_base_address_rupd
       ((!PIC),
        BitsN.fromNat(BitsN.toNat(BitsN.>>+(BitsN.fromNat(pic,40),3)),37)))
  ; L3.for
      (0,127,
       fn i =>
         let
           val f = Map.copy((#config_regs) (!PIC))
           val x0 = BitsN.fromNat(i,7)
           val x1 = Map.lookup(f,BitsN.toNat x0)
         in
           PIC :=
           (PIC_config_regs_rupd
              ((!PIC),
               Map.update
                 (f,BitsN.toNat x0,
                  write'reg'PIC_Config_Reg(x1,BitsN.B(0x0,64)))))
         end)
  ; L3.for
      (0,4,
       fn i =>
         ( let
           ...
           in
             ...
             (PIC_config_regs_rupd
                ((!PIC),
                 Map.update
                   (f,BitsN.toNat x0,PIC_Config_Reg_EN_rupd(x1,true))))
           end
         ; let
             val f = Map.copy((#config_regs) (!PIC))
             val x0 = BitsN.fromNat(i,7)
           ...
           in
             ...
             (PIC_config_regs_rupd
                ((!PIC),
                 Map.update
                   (f,BitsN.toNat x0,
                    PIC_Config_Reg_IRQ_rupd(x1,BitsN.fromNat(i,3)))))
           end
         ))
  ; PIC := (PIC_ip_bits_rupd((!PIC),BitsN.B(0x0,128)))
  ; PIC := (PIC_external_intrs_rupd((!PIC),BitsN.B(0x0,64)))
  ; PIC_update ()
  );

fun PIC_load addr =
  let
    val offset = BitsN.-(addr,(#base_address) (!PIC))
  in
    let
    ...
    in
      ( if BitsN.<(offset,BitsN.B(0x80,37))
          ...
          then ret :=
               (BitsN.||
                  (BitsN.bits((#ip_bits) (!PIC),63,0),
                   (#external_intrs) (!PIC)))
        else if offset = (BitsN.B(0x401,37))
          then ret := (BitsN.bits((#ip_bits) (!PIC),127,64))
...
fun PIC_store (addr,(mask,data)) =
  ...
  in
    ( if BitsN.<(offset,BitsN.B(0x80,37))
        then let
               val f = Map.copy((#config_regs) (!PIC))
               val x0 = BitsN.fromNat(BitsN.toNat offset,7)
               val x1 = Map.lookup(f,BitsN.toNat x0)
             in
               PIC :=
               (PIC_config_regs_rupd
                  ((!PIC),
                   Map.update
                     (f,BitsN.toNat x0,write'reg'PIC_Config_Reg(x1,data))))
             end
               ...
               (PIC_ip_bits_rupd
                  ((!PIC),
                   BitsN.bitFieldInsert
                     (w,
                      BitsN.||
                        (BitsN.bits((#ip_bits) (!PIC),127,64),
                         BitsN.&&(data,mask)),127,64)))
             end
  ...
  in
    if not(((#RI)((#control) (!JTAG_UART))) = readIntr)
      then ( let
           ...
           ; let
               val w = (#external_intrs) (!PIC)
             in
               PIC :=
               (PIC_external_intrs_rupd
                  ((!PIC),
...
fun CPR (n,(reg,sel)) =
  case (n,(reg,sel)) of
   ...
   | (0,(BitsN.B(0x4,5),BitsN.B(0x0,3))) => reg'Context((#Context) (!CP0))
  ...
  ; case (n,(reg,sel)) of
       (0,(BitsN.B(0x0,5),BitsN.B(0x0,3))) =>
          ...
          in
            CP0 :=
            (CP0_Index_rupd
               ((!CP0),Index_Index_rupd(x0,BitsN.bits(value,7,0))))
          end
     | (0,(BitsN.B(0x2,5),BitsN.B(0x0,3))) =>
          let
            val x0 = (#EntryLo0) (!CP0)
     ...
     | (0,(BitsN.B(0x4,5),BitsN.B(0x0,3))) =>
          ...
          in
            CP0 :=
            (CP0_Context_rupd
               ((!CP0),Context_PTEBase_rupd(x0,BitsN.bits(value,63,23))))
          end
     | (0,(BitsN.B(0x4,5),BitsN.B(0x2,3))) =>
          CP0 := (CP0_UsrLocal_rupd((!CP0),value))
     | (0,(BitsN.B(0x5,5),BitsN.B(0x0,3))) =>
          let
            val x0 = (#PageMask) (!CP0)
          in
     ...
     | (0,(BitsN.B(0x6,5),BitsN.B(0x0,3))) =>
          ( let
            ...
            in
              CP0 :=
              (CP0_Wired_rupd
                 ((!CP0),Wired_Wired_rupd(x0,BitsN.bits(value,7,0))))
            end
          ; let
              val x0 = (#Random) (!CP0)
            in
     ...
     | (0,(BitsN.B(0x7,5),BitsN.B(0x0,3))) =>
          ( let
            ...
            in
              CP0 :=
              (CP0_HWREna_rupd
                 ((!CP0),HWREna_CPUNum_rupd(x0,BitsN.bit(value,0))))
            end
          ; let
              val x0 = (#HWREna) (!CP0)
            in
              CP0 :=
              (CP0_HWREna_rupd
                 ((!CP0),HWREna_CC_rupd(x0,BitsN.bit(value,2))))
            end
          ; let
              val x0 = (#HWREna) (!CP0)
            in
              CP0 :=
              (CP0_HWREna_rupd
                 ((!CP0),HWREna_CCRes_rupd(x0,BitsN.bit(value,3))))
            end
          ; let
              val x0 = (#HWREna) (!CP0)
            in
              CP0 :=
              (CP0_HWREna_rupd
                 ((!CP0),HWREna_UL_rupd(x0,BitsN.bit(value,29))))
            end
          )
     | (0,(BitsN.B(0x9,5),BitsN.B(0x0,3))) =>
     ...
     | (0,(BitsN.B(0xB,5),BitsN.B(0x0,3))) =>
          ...
          ; let
            ...
            in
              CP0 :=
              (CP0_Cause_rupd((!CP0),CauseRegister_TI_rupd(x0,false)))
            end
          )
     | (0,(BitsN.B(0xC,5),BitsN.B(0x0,3))) =>
          let
     ...
     | (0,(BitsN.B(0x10,5),BitsN.B(0x2,3))) =>
          ...
          in
            CP0 :=
            (CP0_Config2_rupd
               ((!CP0),ConfigRegister2_SU_rupd(x0,BitsN.bits(value,15,12))))
          end
     | (0,(BitsN.B(0x10,5),BitsN.B(0x6,3))) =>
          let
            val x0 = (#Config6) (!CP0)
          in
            CP0 :=
            (CP0_Config6_rupd
               ((!CP0),ConfigRegister6_LTLB_rupd(x0,BitsN.bit(value,2))))
          end
     | (0,(BitsN.B(0x14,5),BitsN.B(0x0,3))) =>
          let
            val x0 = (#XContext) (!CP0)
          in
            CP0 :=
            (CP0_XContext_rupd
               ((!CP0),XContext_PTEBase_rupd(x0,BitsN.bits(value,63,33))))
          end
     | (0,(BitsN.B(0x17,5),BitsN.B(0x0,3))) =>
  ...
  in
    ...
    ; let
      ...
      in
        CP0 :=
        (CP0_Context_rupd
           ((!CP0),Context_BadVPN2_rupd(x0,BitsN.bits(vAddr,31,13))))
      end
    ...
    in
      ( if a = ((#base_address) (!JTAG_UART))
          then ( ret :=
                 (BitsN.@@
                    (flip_endian_word
                       (reg'JTAG_UART_data((#data) (!JTAG_UART))),
                     flip_endian_word
                       (reg'JTAG_UART_control((#control) (!JTAG_UART)))))
               ; if (BitsN.bits(pAddr,2,0)) = (BitsN.B(0x0,3))
                   then JTAG_UART_load ()
                 else ()
               )
        else if (BitsN.>=(a,(#base_address) (!PIC))) andalso
           (BitsN.<(a,BitsN.+((#base_address) (!PIC),BitsN.B(0x430,37))))
          then ret := (PIC_load a)
        else ret := (Map.lookup((!MEM),BitsN.toNat a))
      ; (!ret)
      )
    end
  end;
...
fun StoreMemory (CCA,(AccessLength,(MemElem,(pAddr,(vAddr,IorD))))) =
  ...
  in
    ...
    ; if a = ((#base_address) (!JTAG_UART))
        then JTAG_UART_store(mask,MemElem)
      else if (BitsN.>=(a,(#base_address) (!PIC))) andalso
         (BitsN.<(a,BitsN.+((#base_address) (!PIC),BitsN.B(0x430,37))))
        then PIC_store(a,(mask,MemElem))
      else MEM :=
           (Map.update
              ((!MEM),BitsN.toNat a,
               BitsN.||
...
fun dfn'TLBP () =
  ...
  else case LookupTLB((#R)((#EntryHi) (!CP0)),(#VPN2)((#EntryHi) (!CP0))) of
          [] =>
             ( let
                 val x0 = (#Index) (!CP0)
               in
             ...
             ; let
               ...
               in
                 CP0 :=
                 (CP0_Index_rupd
                    ((!CP0),Index_Index_rupd(x0,BitsN.B(0x0,8))))
               end
             )
        | [(i,e)] =>
           ( let
    ...
    then SignalException CpU
         ...
         val e =
           if Nat.>=(BitsN.toNat i,TLBEntries)
             then Map.lookup((!TLB_direct),BitsN.toNat(BitsN.bits(i,6,0)))
           else Map.lookup
                  ((!TLB_assoc),
                   BitsN.toNat(BitsN.fromNat(BitsN.toNat i,4)))
       in
         ( let
             val x0 = (#PageMask) (!CP0)
           in
         ...
         ; let
             val x0 = (#EntryLo1) (!CP0)
           in
             CP0 := (CP0_EntryLo1_rupd((!CP0),EntryLo_C_rupd(x0,(#C1) e)))
           end
         ; let
             val x0 = (#EntryLo1) (!CP0)
           in
             CP0 := (CP0_EntryLo1_rupd((!CP0),EntryLo_D_rupd(x0,(#D1) e)))
           end
         ; let
             val x0 = (#EntryLo1) (!CP0)
           in
             CP0 := (CP0_EntryLo1_rupd((!CP0),EntryLo_V_rupd(x0,(#V1) e)))
           end
         ; let
             val x0 = (#EntryLo1) (!CP0)
           in
             CP0 := (CP0_EntryLo1_rupd((!CP0),EntryLo_G_rupd(x0,(#G) e)))
           end
         ; let
             val x0 = (#EntryLo0) (!CP0)
           in
         ...
         ; let
             val x0 = (#EntryLo0) (!CP0)
           in
             CP0 := (CP0_EntryLo0_rupd((!CP0),EntryLo_C_rupd(x0,(#C0) e)))
           end
         ; let
             val x0 = (#EntryLo0) (!CP0)
           in
             CP0 := (CP0_EntryLo0_rupd((!CP0),EntryLo_D_rupd(x0,(#D0) e)))
           end
         ; let
             val x0 = (#EntryLo0) (!CP0)
           in
             CP0 := (CP0_EntryLo0_rupd((!CP0),EntryLo_V_rupd(x0,(#V0) e)))
           end
         ; let
             val x0 = (#EntryLo0) (!CP0)
           in
             CP0 := (CP0_EntryLo0_rupd((!CP0),EntryLo_G_rupd(x0,(#G) e)))
           end
         )
       end;
  ...
  if (not((#CU0)((#Status) (!CP0)))) andalso (not(KernelMode ()))
    ...
    then let
         ...
         in
           ...
           ; if ((#V0) old) andalso ((#V1) old)
               then let
                    ...
                    in
                      TLB_assoc :=
                      (Map.update((!TLB_assoc),BitsN.toNat x,old))
                    end
             else ()
           )
         end
       ...
       in
         TLB_assoc :=
         (Map.update
            ((!TLB_assoc),BitsN.toNat x,
             ModifyTLB
               (Map.lookup
                  ((!TLB_assoc),
                   BitsN.toNat(BitsN.fromNat(BitsN.toNat j,4))))))
       end;
...
fun dfn'RDHWR (rt,rd) =
  if (((#CU0)((#Status) (!CP0))) orelse (KernelMode ())) orelse
    ...
    then case rd of
            BitsN.B(0x0,5) => write'GPR(BitsN.B(0x0,64),rt)
          | BitsN.B(0x2,5) =>
  ...
  ; if ((#Compare) (!CP0)) = ((#Count) (!CP0))
      then ( let
           ...
           ; let
             ...
             in
               CP0 :=
               (CP0_Cause_rupd((!CP0),CauseRegister_TI_rupd(x0,true)))
             end
           )
    else ()
               ...
               (CP0_Cause_rupd
                  ((!CP0),
                   CauseRegister_IP_rupd
                     (x0,
                      BitsN.bitFieldInsert
                        (w,BitsN.bits((#mips_ip_bits) (!PIC),4,0),6,2))))
             end
           ; let
               val x0 = (#Cause) (!CP0)
               val w = (#IP) x0
                   ...
                   CauseRegister_IP_rupd
                     (x0,
                      BitsN.bitFieldInsert
                        (w,
                         BitsN.fromBit
                           ((BitsN.bit((#IP)((#Cause) (!CP0)),7)) orelse
                            (BitsN.bit((#mips_ip_bits) (!PIC),5))),7,7))))
             end
           ; if not((BitsN.&&
...
fun initMips (pc,uart) =
  ( let
      val x0 = (#Config) (!CP0)
    in
      CP0 := (CP0_Config_rupd((!CP0),ConfigRegister_M_rupd(x0,true)))
    end
  ; let
      val x0 = (#Config) (!CP0)
    in
  ...
  ; let
    ...
    in
      CP0 :=
      (CP0_Config_rupd((!CP0),ConfigRegister_AR_rupd(x0,BitsN.B(0x0,3))))
    end
  ; let
      val x0 = (#Config) (!CP0)
    in
      CP0 :=
      (CP0_Config_rupd((!CP0),ConfigRegister_AT_rupd(x0,BitsN.B(0x2,2))))
    end
  ; let
      val x0 = (#Config1) (!CP0)
    in
      CP0 := (CP0_Config1_rupd((!CP0),ConfigRegister1_M_rupd(x0,true)))
    end
  ; let
      val x0 = (#Config1) (!CP0)
    in
      CP0 :=
      (CP0_Config1_rupd
         ((!CP0),ConfigRegister1_MMUSize_rupd(x0,BitsN.B(0xF,6))))
    end
  ; let
      val x0 = (#Config1) (!CP0)
    in
      CP0 :=
      (CP0_Config1_rupd((!CP0),ConfigRegister1_IS_rupd(x0,BitsN.B(0x3,3))))
    end
  ; let
      val x0 = (#Config1) (!CP0)
    in
      CP0 :=
      (CP0_Config1_rupd((!CP0),ConfigRegister1_IL_rupd(x0,BitsN.B(0x4,3))))
    end
  ; let
      val x0 = (#Config1) (!CP0)
    in
      CP0 :=
      (CP0_Config1_rupd((!CP0),ConfigRegister1_IA_rupd(x0,BitsN.B(0x0,3))))
    end
  ; let
      val x0 = (#Config1) (!CP0)
    in
      CP0 :=
      (CP0_Config1_rupd((!CP0),ConfigRegister1_DS_rupd(x0,BitsN.B(0x3,3))))
    end
  ; let
      val x0 = (#Config1) (!CP0)
    in
      CP0 :=
      (CP0_Config1_rupd((!CP0),ConfigRegister1_DL_rupd(x0,BitsN.B(0x4,3))))
    end
  ; let
      val x0 = (#Config1) (!CP0)
    in
      CP0 :=
      (CP0_Config1_rupd((!CP0),ConfigRegister1_DA_rupd(x0,BitsN.B(0x0,3))))
    end
  ; let
      val x0 = (#Config1) (!CP0)
    in
      CP0 := (CP0_Config1_rupd((!CP0),ConfigRegister1_C2_rupd(x0,false)))
    end
  ; let
      val x0 = (#Config1) (!CP0)
    in
      CP0 := (CP0_Config1_rupd((!CP0),ConfigRegister1_MD_rupd(x0,false)))
    end
  ; let
      val x0 = (#Config1) (!CP0)
    in
      CP0 := (CP0_Config1_rupd((!CP0),ConfigRegister1_PC_rupd(x0,false)))
    end
  ; let
      val x0 = (#Config1) (!CP0)
    in
      CP0 := (CP0_Config1_rupd((!CP0),ConfigRegister1_WR_rupd(x0,false)))
    end
  ; let
      val x0 = (#Config1) (!CP0)
    in
      CP0 := (CP0_Config1_rupd((!CP0),ConfigRegister1_CA_rupd(x0,false)))
    end
  ; let
      val x0 = (#Config1) (!CP0)
    in
      CP0 := (CP0_Config1_rupd((!CP0),ConfigRegister1_EP_rupd(x0,false)))
    end
  ; let
      val x0 = (#Config1) (!CP0)
    in
      CP0 := (CP0_Config1_rupd((!CP0),ConfigRegister1_FP_rupd(x0,false)))
    end
  ; let
      val x0 = (#Config2) (!CP0)
    in
      CP0 := (CP0_Config2_rupd((!CP0),ConfigRegister2_M_rupd(x0,true)))
    end
  ; let
      val x0 = (#Config2) (!CP0)
    in
      CP0 :=
      (CP0_Config2_rupd((!CP0),ConfigRegister2_TU_rupd(x0,BitsN.B(0x0,3))))
    end
  ; let
      val x0 = (#Config2) (!CP0)
    in
      CP0 :=
      (CP0_Config2_rupd((!CP0),ConfigRegister2_TS_rupd(x0,BitsN.B(0x0,4))))
    end
  ; let
      val x0 = (#Config2) (!CP0)
    in
      CP0 :=
      (CP0_Config2_rupd((!CP0),ConfigRegister2_TL_rupd(x0,BitsN.B(0x0,4))))
    end
  ; let
      val x0 = (#Config2) (!CP0)
    in
      CP0 :=
      (CP0_Config2_rupd((!CP0),ConfigRegister2_TA_rupd(x0,BitsN.B(0x0,4))))
    end
  ; let
      val x0 = (#Config2) (!CP0)
    in
      CP0 :=
      (CP0_Config2_rupd((!CP0),ConfigRegister2_SU_rupd(x0,BitsN.B(0x3,4))))
    end
  ; let
      val x0 = (#Config2) (!CP0)
    in
      CP0 :=
      (CP0_Config2_rupd((!CP0),ConfigRegister2_SS_rupd(x0,BitsN.B(0x8,4))))
    end
  ; let
      val x0 = (#Config2) (!CP0)
    in
      CP0 :=
      (CP0_Config2_rupd((!CP0),ConfigRegister2_SL_rupd(x0,BitsN.B(0x4,4))))
    end
  ; let
      val x0 = (#Config2) (!CP0)
    in
      CP0 :=
      (CP0_Config2_rupd((!CP0),ConfigRegister2_SA_rupd(x0,BitsN.B(0x0,4))))
    end
  ; let
      val x0 = (#Config3) (!CP0)
    in
      CP0 := (CP0_Config3_rupd((!CP0),ConfigRegister3_M_rupd(x0,true)))
    end
  ; let
      val x0 = (#Config3) (!CP0)
    in
      CP0 := (CP0_Config3_rupd((!CP0),ConfigRegister3_ULRI_rupd(x0,true)))
    end
  ; let
      val x0 = (#Config3) (!CP0)
    in
      CP0 :=
      (CP0_Config3_rupd((!CP0),ConfigRegister3_DSPP_rupd(x0,false)))
    end
  ; let
      val x0 = (#Config3) (!CP0)
    in
      CP0 := (CP0_Config3_rupd((!CP0),ConfigRegister3_LPA_rupd(x0,false)))
    end
  ; let
      val x0 = (#Config3) (!CP0)
    in
      CP0 :=
      (CP0_Config3_rupd((!CP0),ConfigRegister3_VEIC_rupd(x0,false)))
    end
  ; let
      val x0 = (#Config3) (!CP0)
    in
      CP0 :=
      (CP0_Config3_rupd((!CP0),ConfigRegister3_VInt_rupd(x0,false)))
    end
  ; let
      val x0 = (#Config3) (!CP0)
    in
      CP0 := (CP0_Config3_rupd((!CP0),ConfigRegister3_SP_rupd(x0,false)))
    end
  ; let
      val x0 = (#Config3) (!CP0)
    in
      CP0 := (CP0_Config3_rupd((!CP0),ConfigRegister3_MT_rupd(x0,false)))
    end
  ; let
      val x0 = (#Config3) (!CP0)
    in
      CP0 := (CP0_Config3_rupd((!CP0),ConfigRegister3_SM_rupd(x0,false)))
    end
  ; let
      val x0 = (#Config3) (!CP0)
    in
      CP0 := (CP0_Config3_rupd((!CP0),ConfigRegister3_TL_rupd(x0,false)))
    end
  ; let
      val x0 = (#Config6) (!CP0)
    in
      CP0 :=
      (CP0_Config6_rupd
         ((!CP0),ConfigRegister6_TLBSize_rupd(x0,BitsN.B(0x8F,16))))
    end
  ; let
      val x0 = (#Config6) (!CP0)
    in
      CP0 :=
      (CP0_Config6_rupd((!CP0),ConfigRegister6_LTLB_rupd(x0,false)))
    end
  ; let
      val x0 = (#Status) (!CP0)
    in
      CP0 :=
      (CP0_Status_rupd
         ((!CP0),write'reg'StatusRegister(x0,BitsN.B(0x0,32))))
    end
  ; let
      val x0 = (#Status) (!CP0)
    in
      CP0 := (CP0_Status_rupd((!CP0),StatusRegister_BEV_rupd(x0,true)))
    end
  ; let
      val x0 = (#Status) (!CP0)
    in
  ...
  ; let
      val x0 = (#Status) (!CP0)
    in
      CP0 := (CP0_Status_rupd((!CP0),StatusRegister_EXL_rupd(x0,false)))
    end
  ; let
      val x0 = (#Status) (!CP0)
    in
      CP0 := (CP0_Status_rupd((!CP0),StatusRegister_ERL_rupd(x0,false)))
    end
  ; let
      val x0 = (#Status) (!CP0)
    in
      CP0 := (CP0_Status_rupd((!CP0),StatusRegister_KX_rupd(x0,true)))
    end
  ; let
      val x0 = (#Status) (!CP0)
    in
      CP0 := (CP0_Status_rupd((!CP0),StatusRegister_SX_rupd(x0,true)))
    end
  ; let
      val x0 = (#Status) (!CP0)
    in
      CP0 := (CP0_Status_rupd((!CP0),StatusRegister_UX_rupd(x0,true)))
    end
  ; CP0 := (CP0_Count_rupd((!CP0),BitsN.B(0x0,32)))
  ; CP0 := (CP0_Compare_rupd((!CP0),BitsN.B(0x0,32)))
  ...
  ; let
      val x0 = (#Index) (!CP0)
    in
      CP0 := (CP0_Index_rupd((!CP0),Index_Index_rupd(x0,BitsN.B(0x0,8))))
    end
  ; let
      val x0 = (#Random) (!CP0)
    in
      ...
      (CP0_Random_rupd
         ((!CP0),
          Random_Random_rupd(x0,BitsN.fromNat(Nat.-(TLBEntries,1),8))))
    end
  ; let
      val x0 = (#Wired) (!CP0)
    in
      CP0 := (CP0_Wired_rupd((!CP0),Wired_Wired_rupd(x0,BitsN.B(0x0,8))))
    end
  ; let
      val x0 = (#HWREna) (!CP0)
    in
      CP0 :=
      (CP0_HWREna_rupd((!CP0),write'reg'HWREna(x0,BitsN.B(0x0,32))))
    end
  ...
  ; PIC_initialise 2139111424
  );

fun done () =
  case (!log) of [w_c0(BitsN.B(0x17,5),_)] => true | _ => false;
