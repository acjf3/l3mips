(* mips - generated by L<3> - Wed Sep 10 15:11:38 2014 *)
...
fun StoreMemory (CCA,(AccessLength,(MemElem,(pAddr,(vAddr,IorD))))) =
  ...
  in
    ...
    ; let
      ...
      in
        ( if a = ((#base_address) (!JTAG_UART))
            then ( found := true; JTAG_UART_store(mask,MemElem) )
          else L3.for
                 (0,Nat.-((!totalCore),1),
                  fn core =>
                    if (BitsN.>=
                          (a,
                           Map.lookup
                             ((!PIC_base_address),
                              BitsN.toNat(BitsN.fromNat(core,8))))) andalso
                       (BitsN.<
                          (a,
                           BitsN.+
                             (Map.lookup
                                ((!PIC_base_address),
                                 BitsN.toNat(BitsN.fromNat(core,8))),
                              BitsN.B(0x430,37))))
                      then ( found := true
                           ; PIC_store
                               (BitsN.fromNat(core,8),(a,(mask,MemElem)))
                           )
                    else ())
        ; if (!found) = false
            then ( L3.for
