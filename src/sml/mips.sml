(* mips - generated by L<3> - Thu Oct 02 13:20:50 2014 *)
...
fun w_gpr (r,data) =
  String.concat
    ["Reg ",Nat.toString(BitsN.toNat r)," <- 0x",BitsN.toHexString data];

fun w_hi data = ("HI <- 0x") ^ (BitsN.toHexString data);

fun w_lo data = ("LO <- 0x") ^ (BitsN.toHexString data);

fun w_c0 (r,data) = String.concat[cpr r," <- 0x",BitsN.toHexString data];

fun w_mem (pAddr,(mask,(sz,data))) =
  String.concat
    ["Address 0x",BitsN.toHexString pAddr," <- 0x",BitsN.toHexString data,
     " [",Nat.toString(BitsN.toNat sz)," bytes], mask 0x",
     BitsN.toHexString mask];
...
fun pad (width,n) =
  if Nat.>(width,0)
    then (if Nat.>(n,0) then "" else "0")
           ^
           (pad(Nat.-(width,1),Nat.div(n,16)))
  else "";

fun hex64 x =
  (pad(16,BitsN.toNat x))
    ^
    (if x = (BitsN.B(0x0,64)) then "" else BitsN.toHexString x);
...
fun write'GPR (value,n) =
  if not(n = (BitsN.B(0x0,5)))
    then ( write'gpr(value,n); mark(2,w_gpr(n,value)) )
  else ();

fun dumpRegs () =
  ( mark(0,"======   Registers   ======")
  ; mark(0,("Core = ") ^ (Nat.toString(BitsN.toNat (!procID))))
  ; mark(0,("PC     ") ^ (hex64(PC ())))
  ; L3.for
      (0,31,
       fn i =>
...
fun write'HI value = ( write'hi(Option.SOME value); mark(2,w_hi value) );
...
fun write'LO value = ( write'lo(Option.SOME value); mark(2,w_lo value) );
...
fun Fetch () =
  ( log := (Map.update((!log),0,[]))
  ; log := (Map.update((!log),2,[]))
  ; let
      val x = CP0 ()
      val x0 = (#Random) x
    in
