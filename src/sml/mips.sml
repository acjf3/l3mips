(* mips - generated by L<3> - Fri Sep 19 13:32:45 2014 *)
...
fun LoadMemory (AccessLength,(vAddr,(IorD,AccessType))) =
  let
           ...
           ((!pAddr),
            BitsN.??
              (BitsN.bits((!pAddr),2,0),
               BitsN.resize_replicate 3 (ReverseEndian (),3)),2,0))
      ; pAddr :=
        (if BigEndianMem ()
           then (!pAddr)
               ...
               in
                 ...
                 in
                   ...
                   in
                     ...
                     ; if (!found) = false
                         then ret := (Map.lookup((!MEM),BitsN.toNat a))
                       else ()
                     ; ((!ret),(!pAddr))
                     )
                   end
...
fun StoreMemory (AccessLength,(MemElem,(vAddr,(IorD,AccessType)))) =
  let
    val pAddr = ref (BitsN.B(0x0,40))
  in
    let
...
fun loadByte (base,(rt,(offset,unsigned))) =
  let
    ...
    val (memdoubleword,pAddr) = LoadMemory(BYTE,(vAddr,(DATA,LOAD)))
  in
    if not(exceptionSignalled ())
      then let
             val byte =
    ...
    if BitsN.bit(vAddr,0)
      then ( let
           ...
           val (memdoubleword,pAddr) =
             LoadMemory(HALFWORD,(vAddr,(DATA,LOAD)))
         in
           if not(exceptionSignalled ())
             then let
                    val byte =
  ...
  in
    if not((BitsN.bits(vAddr,1,0)) = (BitsN.B(0x0,2)))
      then ( let
           ...
           val (memdoubleword,pAddr) =
             LoadMemory(WORD,(vAddr,(DATA,LOAD)))
         in
           if not(exceptionSignalled ())
             then let
                    val byte =
  ...
  in
    if not((BitsN.bits(vAddr,2,0)) = (BitsN.B(0x0,3)))
      then ( let
           ...
           val (memdoubleword,pAddr) =
             LoadMemory(DOUBLEWORD,(vAddr,(DATA,LOAD)))
         in
           if not(exceptionSignalled ())
...
fun dfn'LWL (base,(rt,offset)) =
  let
    ...
    val (memdoubleword,pAddr) =
      LoadMemory(BitsN.@@(BitsN.B(0x0,1),byte),(vAddr,(DATA,LOAD)))
  in
    if not(exceptionSignalled ())
      then let
             val temp =
    ...
    val byte =
      BitsN.??
        (BitsN.bits(vAddr,2,0),
         BitsN.resize_replicate 3 (BigEndianCPU (),3))
    val (memdoubleword,pAddr) =
      LoadMemory(BitsN.-(DOUBLEWORD,byte),(vAddr,(DATA,LOAD)))
  in
    if not(exceptionSignalled ())
      then ( write'GPR
               (case byte of
...
fun dfn'SB (base,(rt,offset)) =
  let
    ...
    val pAddr = StoreMemory(BYTE,(datadoubleword,(vAddr,(DATA,STORE))))
  in
  ...
  in
    if BitsN.bit(vAddr,0)
      then ( let
           ...
           val pAddr =
             StoreMemory(HALFWORD,(datadoubleword,(vAddr,(DATA,STORE))))
         in
           if not(exceptionSignalled ()) then write'LLbit NONE else ()
         end
  end;
    ...
    if not((BitsN.bits(vAddr,1,0)) = (BitsN.B(0x0,2)))
      then ( let
           ...
           val datadoubleword =
             BitsN.<<(GPR rt,Nat.*(8,BitsN.toNat bytesel))
           val pAddr =
             StoreMemory(WORD,(datadoubleword,(vAddr,(DATA,STORE))))
         in
           ()
         end
  end;
  ...
  in
    if not((BitsN.bits(vAddr,2,0)) = (BitsN.B(0x0,3)))
      then ( let
           ...
           val pAddr =
             StoreMemory(DOUBLEWORD,(datadoubleword,(vAddr,(DATA,STORE))))
         in
           ()
         end
  end;
...
fun initMips (pc,uart) =
  ...
  ; if (!procID) = (BitsN.B(0x0,8))
      then ( JTAG_UART_initialise uart
