    ...
    match i
    {
        case COP1(j) =>
            match j
            {
                ...
                case MOVF(rd, rs, cc) => op2r("movf", rd, rs)
                case MOVF_D(fd, fs, cc) => op2r("movf.d", fd, fs)
                case MOVF_S(fd, fs, cc) => op2r("movf.s", fd, fs)
                case MOVN_D(fd, fs, rt) => op3r("movn.d", fd, fs, rt)
                case MOVN_S(fd, fs, rt) => op3r("movn.s", fd, fs, rt)
                case MOVT(rd, rs, cc) => op2r("movt", rd, rs)
                case MOVT_D(fd, fs, cc) => op2r("movt.d", fd, fs)
                case MOVT_S(fd, fs, cc) => op2r("movt.s", fd, fs)
