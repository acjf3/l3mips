...
string instructionToString (i::instruction) =
   match i
   {
     ...
     case Branch (BLTZ (rs, imm))        => op1rai ("bltz",rs,imm)
     case Branch (BGEZ (rs, imm))        => op1rai ("bgez",rs,imm)
     case Branch (BLTZL (rs, imm))       => op1rlai ("bltzl",rs,imm)
     case Branch (BGEZL (rs, imm))       => op1rlai ("bgezl",rs,imm)
     ...
     case Branch (BLTZAL (rs, imm))      => op1rai ("bltzal",rs,imm)
     case Branch (BGEZAL (rs, imm))      => op1rai ("bgezal",rs,imm)
     case Branch (BLTZALL (rs, imm))     => op1rlai ("bltzall",rs,imm)
     case Branch (BGEZALL (rs, imm))     => op1rlai ("bgezall",rs,imm)
     case Branch (J (imm))               => op1ai ("j",imm)
     case Branch (JAL (imm))             => op1ai ("jal",imm)
     ...
     case Branch (BEQ (0, 0, imm))       => op1ai ("b",imm)
     case Branch (BEQ (rs, rt, imm))     => op2rai ("beq",rs,rt,imm)
     case Branch (BNE (rs, rt, imm))     => op2rai ("bne",rs,rt,imm)
     case Branch (BLEZ (rs, imm))        => op1rai ("blez",rs,imm)
     case Branch (BGTZ (rs, imm))        => op1rai ("bgtz",rs,imm)
     ...
     case Branch (BEQL (rs, rt, imm))    => op2rlai ("beql",rs,rt,imm)
     case Branch (BNEL (rs, rt, imm))    => op2rlai ("bnel",rs,rt,imm)
     case Branch (BLEZL (rs, imm))       => op1rlai ("blezl",rs,imm)
     case Branch (BGTZL (rs, imm))       => op1rlai ("bgtzl",rs,imm)
     ...
     case CACHE (rs, opn, imm)           => "cache ":ihex(opn):", ":ihex(imm):"(":reg_name(rs):")"
     case SYSCALL                        => "syscall"
     case BREAK                          => "break"
     case SYNC (imm5)                    => "sync ":ihex(imm5)
     case TLBR                           => "tlbr"
     case TLBWI                          => "tlbwi"
     case TLBWR                          => "tlbwr"
     case TLBP                           => "tlbp"
